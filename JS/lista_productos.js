...
function htmlProducto(producto, divProductos) {
  let maxPalabras = 11;
  let palabras = producto.descripcion.split(" ");
  let descripcionCorta =
    palabras.length > maxPalabras
...
function mostrarProductosSeleccionados() {
  ...
  let categorias = Array.from(categoriasSelecionados.selectedOptions).map(
    (option) => option.value
  );
  ...
  productos
    .filter(
      (producto) =>
        categoriasSelecionados.value === "todos" ||
